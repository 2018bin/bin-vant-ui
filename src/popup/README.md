...
```html
<bin-cell is-link @click="showPopup">展示弹出层</bin-cell>
<bin-popup v-model="show">内容</bin-popup>
```
...
```html
<bin-popup v-model="show" position="top" :style="{ height: '30%' }" />
```
...
```html
<!-- 挂载到 body 节点下 -->
<bin-popup v-model="show" teleport="body" />

<!-- 挂载到 #app 节点下 -->
<bin-popup v-model="show" teleport="#app" />

<!-- 挂载到指定的元素下 -->
<bin-popup v-model="show" :teleport="myContainer" />
```
